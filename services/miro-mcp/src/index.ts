...
async function handleToolCall(name: string, args: Json): Promise<unknown> {
  switch (name) {
    // Board Management
    ...
    case "miro_create_shape": {
      ...
      if (args.color) shapeData.style = { fillColor: args.color };
      return miroPost(`/v2/boards/${args.board_id}/shapes`, shapeData);
    }

    case "miro_update_shape": {
      const updateData: Json = {};
      if (args.content !== undefined) updateData.data = { content: args.content };
      if (args.x !== undefined || args.y !== undefined) {
        updateData.position = { x: args.x, y: args.y };
      }
    ...
    case "miro_create_text": {
      ...
      if (args.width) textData.geometry = { width: args.width };
      if (args.font_size || args.text_align) {
        textData.style = {};
        if (args.font_size) (textData.style as Json).fontSize = String(args.font_size);
        if (args.text_align) (textData.style as Json).textAlign = args.text_align;
      }
    ...
    case "miro_update_text": {
      ...
      if (args.font_size) updateData.style = { fontSize: String(args.font_size) };
    ...
    case "miro_create_frame": {
      ...
      if (args.color) frameData.style = { fillColor: args.color };
      return miroPost(`/v2/boards/${args.board_id}/frames`, frameData);
    }

    // Connectors
    case "miro_create_connector": {
      const connectorData: Json = {
        startItem: { id: args.start_item_id },
        endItem: { id: args.end_item_id }
      };
      if (args.caption) connectorData.captions = [{ content: args.caption }];
      if (args.style || args.stroke_color || args.stroke_width) {
        connectorData.style = {};
        if (args.style) (connectorData.style as Json).strokeStyle = args.style;
        if (args.stroke_color) (connectorData.style as Json).strokeColor = args.stroke_color;
        if (args.stroke_width) (connectorData.style as Json).strokeWidth = String(args.stroke_width);
      }
    ...
    case "miro_create_card": {
      ...
      if (args.due_date) (cardData.data as Json).dueDate = args.due_date;
      if (args.assignee_id) (cardData.data as Json).assigneeId = args.assignee_id;
    ...
    case "miro_create_image_from_url": {
      ...
      if (args.width) imageData.geometry = { width: args.width };
      if (args.title) (imageData.data as Json).title = args.title;
      return miroPost(`/v2/boards/${args.board_id}/images`, imageData);
    }

    // Bulk Operations
    case "miro_bulk_create_sticky_notes": {
      const notes = args.notes as Array<{ content: string; x?: number; y?: number; color?: string }>;
      const results = [];
      for (const note of notes) {
        ...
        if (note.color) stickyData.style = { fillColor: note.color };
...
async function miroGet(path: string, query: Record<string, unknown>) {
  ...
  if (!r.ok) throw new Error(`Miro GET ${path} failed: ${r.status} ${await r.text()}`);
  return r.json();
}

async function miroPost(path: string, body: unknown) {
  ...
  if (!r.ok) throw new Error(`Miro POST ${path} failed: ${r.status} ${await r.text()}`);
  return r.json();
}

async function miroPatch(path: string, body: unknown) {
  const url = `https://api.miro.com${path}`;
  const r = await fetch(url, {
    method: "PATCH",
    headers: {
  ...
  if (!r.ok) throw new Error(`Miro PATCH ${path} failed: ${r.status} ${await r.text()}`);
  return r.json();
}

async function miroDelete(path: string) {
  const url = `https://api.miro.com${path}`;
  const r = await fetch(url, {
    method: "DELETE",
    headers: {
  ...
  if (!r.ok) throw new Error(`Miro DELETE ${path} failed: ${r.status} ${await r.text()}`);
