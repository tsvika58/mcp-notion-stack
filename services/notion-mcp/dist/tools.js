import { appendBlocks, archivePage, createPageInDatabase, listBlockChildren, notionCanonicalPageUrl, queryDatabase, retrievePage, search, updatePageProperties } from './notion.js';
import { AppendBlocksInput, ArchivePageInput, CreatePageInput, ListBlockChildrenInput, QueryDatabaseInput, RetrievePageInput, SearchInput, UpdatePageInput } from './types.js';
...
export function registerTools(server) {
    ...
    server.registerTool('notion_create_page', {
        ...
        if (!dbId)
            throw new Error('databaseId not provided and NOTION_DEFAULT_DATABASE_ID is not set.');
        const page = await createPageInDatabase(dbId, titleProperty, title, properties, blocks);
        return {
            content: [
    ...
    server.registerTool('notion_retrieve_page', {
    ...
    server.registerTool('notion_list_block_children', {
