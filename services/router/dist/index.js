...
const PORT = parseInt(process.env.PORT || process.env.ROUTER_PORT || "3032", 10);
const ROUTER_API_KEY = process.env.ROUTER_API_KEY || ""; // optional bearer
function requireAuth(req, res, next) {
    if (!ROUTER_API_KEY)
        return next(); // no auth enforced
...
app.post("/mcp/tools.call", requireAuth, async (req, res) => {
    try {
        ...
        const errorMessage = err instanceof Error ? err.message : "MCP tools.call failed";
...
app.post("/notion/pages.create", requireAuth, async (req, res) => {
    try {
        ...
        if (!database_id || !title) {
            return res
                .status(400)
                .json({ error: "database_id and title are required" });
        }
        ...
        const errorMessage = err instanceof Error ? err.message : "pages.create failed";
        res.status(502).json({ error: errorMessage });
    }
});
app.listen(PORT, () => {
