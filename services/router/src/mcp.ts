...
async function mcpInitialize(): Promise<{
  sessionId: string;
  result: InitResult;
}> {
  ...
  if (!sessionId) throw new Error("No mcp-session-id returned by server");
  ...
  return { sessionId, result: payload.result as InitResult };
}

// Generic SSE JSON-RPC call; returns parsed .result if available, else full message
export async function mcpSseCall<T = any>(
  req: Omit<JsonRpcReq, "id"> & { id?: number | string },
): Promise<{ result: T; raw: any }> {
...
export async function notionCreatePageViaMcp(args: {
  ...
  const { result } = await mcpSseCall<any>(callReq);
